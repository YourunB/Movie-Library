...
export class DashboardEffects {
  ...
  load$ = createEffect(() => {
    const lang$ = toObservable(this.tmbdService.langRequests);
        ...
        lang$.pipe(
          switchMap(() =>
            forkJoin({
              ...
              map(({ trendingPage, topRatedPage, peoplePage }) =>
                DashboardActions.loadDashboardSuccess({
                  trending: trendingPage.results ?? [],
                  topRated: topRatedPage.results ?? [],
                  popularPeople: peoplePage.results ?? [],
                  reviews: [],
                })
              ),
              catchError((err: unknown) =>
                of(
                  DashboardActions.loadDashboardFailure({ error: String(err) })
                )
              )
            )
          )
